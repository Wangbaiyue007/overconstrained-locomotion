...
void robot_init()
{
    ...
    robot.omega = pow(robot.k_s/robot.m, 0.5);
    robot.beta = 0;
    robot.beta_dot = 0;
...
void updateRobotState()
{
    ...
    /*Superviser*/
    /**/
    double *coordinate = robot_coordinates();
    double *x[3];
    for(int i = 0; i < 3; i++)
    {
        x[i] = &coordinate[i];
...
void LegUpdate()
{
    ...
    {
        robot.spring_force = (robot.k_s * (-spring_diff) + robot.k_v * (-robot.spring_diff_v/robot.omega)) / (sqrt(pow(spring_diff, 2) + pow( (robot.spring_diff_v/robot.omega), 2)) + 0.1);
    }
